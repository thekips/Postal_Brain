...
#     '''
#     d_lat_lon = np.abs(np.array(loc1) - np.array(loc2))
#     return np.sum(d_lat_lon)
...
# lng_max = data[:,0].max()
# lng_min = data[:,0].min()
# data[:,0] = (data[:,0] - lng_min) / (lng_max - lng_min)
# data[:,1] = (data[:,1] - lat_min) / (lat_max - lat_min)
...
#     out, h, c, _ = model(x=x, X_all=data, h=h, c=c, mask=mask)
